...
public class MFCC implements AudioProcessor {
	...
	public boolean process(AudioEvent audioEvent) {
		...
		i_sample += audioFloatBuffer.length;
//		System.out.println("deebug: i_sample=" + i_sample);
		for(int i = 0; i < audioFloatBuffer.length; i++) {
			if (audioFloatBuffer[i] > 0.001 || audioFloatBuffer[i] < -0.001) {
				System.out.println("deebug: audio float buffer: " + (i_sample + i) + ", " + audioFloatBuffer[i]);
			}
		}
