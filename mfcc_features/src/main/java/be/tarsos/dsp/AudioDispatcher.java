...
public class AudioDispatcher implements Runnable {
	...
	private void skipToStart() {
		...
		try{
			...
			System.out.println("deebug: skipToStart, bytesProcessed=" + bytesProcessed);
		}catch(IOException e){
	...
	public void stop() {
		stopped = true;
		System.out.println("deebug: bytesProcessed=" + bytesProcessed + ", samplesProcessed=" + bytesProcessed/format.getFrameSize());
		for (final AudioProcessor processor : audioProcessors) {
	...
	private int readNextAudioBlock() throws IOException {
		...
		// deebug: turn it to mono instead of stereo
		if (format.getFrameSize() == 4) {
			//System.out.println("deebug: skip every two bytes");
			for(int i = 0; i < audioByteBuffer.length/4; i++){
				//audioByteBuffer[4*i] = 0;
				//audioByteBuffer[4*i+1] = 0;
				audioByteBuffer[4*i+2] = 0;
				audioByteBuffer[4*i+3] = 0;
			}
		}

		if (endOfStream) {
			System.out.println("deebug: endOfSteam: offsetInBytes=" + offsetInBytes + ", totalBytesRead=" + totalBytesRead);
